...
📁 File structure
/project-folder
  ├─ StudentRecordApp.java     // main single-file Java program
  ├─ students.txt              // data file (CSV)
  └─ Lab Assignment 4.pdf      // assignment spec (referenced locally)
...
103,Karan,karan@mail.com,BCA,76.2
...
Data model: Student inner class with roll, name, email, course, marks.
Methods: fromCSV(String), toCSV(), and toString() for display.

Storage: students.txt (plain CSV). File handling via BufferedWriter/RandomAccessFile.

Collection: ArrayList<Student> is used with generics for type safety.
...
Display: Iteration uses Iterator<Student> to demonstrate Iterator use.
